...
- 当前进度：**迭代 1 / 2 / 3 已全部交付**（真实 `java -jar` 冷启动通过）；**迭代 4 已交付 W4/B-5 合规报告（含后台导出入口）、W1 四层异常埋点补全、W6 个人状态库地基、W6-b 状态库后台、W2 灰度开关与熔断回滚、W3 工具插件化扩展接口**（基线 485 测试全绿，Flyway 至 V1.0.14）
...
| 迭代 4 | 可演进 + 合规自证：W4 B-5 合规报告（含 `/compliance/export` 前端入口）、W1 L1/L2 异常埋点补全、W6 个人状态库地基（表 + 生长 + 溯源 + 召回）、W6-b 状态库后台（只读列表 + 纠错删除）、W2 灰度开关与熔断回滚（白名单 + 稳定分桶放量 + 30s 自动回滚 + 一键回滚/预览端点）、W3 工具插件化扩展接口（工具自述 + 注册表反查，新增工具零改引擎；非法 Schema 阻止启动；示例工具默认暗启动）已交付；FR-09 完整版、B-3/B-4、真 token 流式与事件触发 / 主动执行层规划中 | ✅ W4 + W1 + W6 + W6-b + W2 + W3 已交付（485 测试），其余规划中 |
