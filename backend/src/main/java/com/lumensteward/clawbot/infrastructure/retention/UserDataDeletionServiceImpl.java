...
public class UserDataDeletionServiceImpl implements UserDataDeletionService {
    ...
    public UserDataDeletionServiceImpl(WxMessageMapper messageMapper, WxSessionMapper sessionMapper,
                                      PetProfileMapper petProfileMapper, ToolCallLogMapper toolLogMapper,
    ...
    public DeletionSummary deleteUserData(String openid, Long operatorId, String ip, DeletionScope scope) {
        ...
        auditLogService.record(operatorId, "DATA_DELETE", scope.name(),
                ...
                "用户数据删除请求", ip, 1);
        log.info("用户数据删除完成 openid={} scope={} messages={} sessions={} pets={} anonLogs={} anonUser={}",
                MaskUtils.openid(openid), scope, messages, sessions, pets, anonymizedLogs, anonymizedUser);

        return new DeletionSummary(openid, scope, messages, sessions, pets, anonymizedLogs, anonymizedUser);
    }
}
