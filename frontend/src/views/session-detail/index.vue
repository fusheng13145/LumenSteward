...
<template>
  <section>
    <el-page-header :content="`会话 #${sessionId} 消息流`" @back="router.back()" />
    ...
    <el-form class="filters" inline @submit.prevent="load">
      ...
      <el-form-item label="类型">
        <el-select v-model="filters.msgType" placeholder="全部" clearable style="width: 130px">
    ...
    <el-table v-loading="loading" :data="messages" border size="small">
      ...
      <el-table-column label="角色" width="90">
        <template #default="{ row }">{{ MESSAGE_ROLE_LABELS[row.role] ?? row.role }}</template>
      </el-table-column>
      <el-table-column label="类型" width="90">
        <template #default="{ row }">{{ MESSAGE_TYPE_LABELS[row.msgType] ?? row.msgType }}</template>
      </el-table-column>
